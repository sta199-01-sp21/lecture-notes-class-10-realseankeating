...
Each marble is extremely close to the "true" probability, but it could get closer with more trials. The greater the number of trials, the closer we get to the "true" probability.
...
My guess is that the absolute value of the difference between dice 1 and 2 is equal to 4.

```{r simulate_rolls}
roll_results <- tibble(
  die_1 = replicate(n = 10,000, expr = sample(1:6, size = 1)),
  die_2 = replicate(n = 10,000, expr = sample(1:6, size = 1))
)
```
...
Event 1 is more likely actually, as 0.2>0.1
